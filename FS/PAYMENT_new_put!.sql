...
SELECT E.RECORD_TYPE AS "Направление",
 	CASE WHEN E.EXPENSE_TYPE_ID IN (2) THEN 'Доставка'
		WHEN E.EXPENSE_TYPE_ID IN (20) THEN 'Прочие доходы'
		ELSE NULL
		END	AS "Группа товара",
	IIF(ET.NAME IS NOT NULL, ET.NAME, 'Статья не указана') AS "Статья",
	CASE WHEN E.EXPENSE_TYPE_ID IN (2,20) THEN NULL
		WHEN E.EXPENSE_TYPE_ID IN (5,6,7,10,17,19) THEN 'Переменные'
		WHEN E.EXPENSE_TYPE_ID IN (11,13,16,18,22) THEN 'Постоянные'
...
FROM
	...
	WHERE W.IS_DELETED = 0 AND WI.IS_DELETED = 0 AND SR.SHOP_ID=0 AND W.RECORD_TYPE IN (-1,1) AND ((W.COST = 0) OR (W.PAID > 0))
UNION ALL
...
FROM
	...
	LEFT JOIN SIZED_REMAINDERS SR ON (WI.SIZE_ID = SR.SIZE_ID)
	LEFT JOIN GOOD_GROUPS GG ON (GG.ID = G.GROUP_ID)
	WHERE W.IS_DELETED = 0 AND WI.IS_DELETED = 0 AND SR.SHOP_ID=0 AND W.RECORD_TYPE IN (-1,1) AND (W.PAID < W.COST)
