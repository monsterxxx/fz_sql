SELECT 	MIN(DATEADD(SECOND, W.WAYBILL_DATE, cast('1970-01-01 00:00:00' as timestamp))) AS "Ожидают с",
	--SUP.NAME AS "Клиент",
	...
	--WI.QUANTITY AS "Кол.",
	MAX(SR.REMAINDER) AS "Остаток в FZ",
	...
	SUM(WI.QUANTITY) AS "Заказано",
	IIF(MAX(SR.REMAINDER) - 1 - SUM(WI.QUANTITY)>0,0,MAX(SR.REMAINDER) - 1 - SUM(WI.QUANTITY)) AS "Требуется"
...
FROM WAYBILLS W
	...
	WHERE W.RECORD_TYPE = -1 AND W.IS_DELETED = 0 AND WI.IS_DELETED = 0 AND W.IS_RESERVE = 1
	GROUP BY GG.NAME, G.NAME, S.NAME