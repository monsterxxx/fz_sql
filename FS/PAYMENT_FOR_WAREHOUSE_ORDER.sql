...
SELECT (-1)*RECORD_TYPE,
	...
	IIF(SR.SHOP_ID=0, SR.RESERVED, NULL),
...
FROM
	...
	WHERE W.IS_DELETED = 0 AND WI.IS_DELETED = 0 AND SR.SHOP_ID=0 AND SR.IS_DELETED=0 AND W.RECORD_TYPE IN (-1,1) AND ((W.COST = 0) OR (W.PAID > 0))
UNION ALL
...
FROM
	...
	LEFT JOIN SIZED_REMAINDERS SR ON (WI.SIZE_ID = SR.SIZE_ID)
	LEFT JOIN GOOD_GROUPS GG ON (GG.ID = G.GROUP_ID)
	WHERE W.IS_DELETED = 0 AND WI.IS_DELETED = 0 AND SR.SHOP_ID=0 AND SR.IS_DELETED=0 AND W.RECORD_TYPE IN (-1,1) AND (W.PAID < W.COST)